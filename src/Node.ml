type style = <
   setProperty : Js.Json.t Js.undefined [@bs.get]; (* TODO:  Revamp this and the next line... *)
...
type t = <
  ...
  addEventListener : string -> t Event.cb -> Event.options -> unit [@bs.meth];
  removeEventListener : string -> t Event.cb -> Event.options -> unit [@bs.meth];
...
type event = t Event.t

type event_cb = t Event.cb


external getProp_asEventListener : t -> 'key -> t Event.cb Js.undefined = "" [@@bs.get_index]

external setProp_asEventListener : t -> 'key -> t Event.cb Js.undefined -> unit = "" [@@bs.set_index]
